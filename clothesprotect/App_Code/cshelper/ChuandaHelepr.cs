...
public static  class ChuanDaHelper
{
  ...
  public Result GetInfoByTypeOrName(int styleId,string name,int userid)
  {
        var result = new Result<List<Chuanda>>();
        var whereexp=" userid="+userid;
        if(typeid!=-1){
whereexp=whereexp+" and styleId="+typeid;
        }
...
public Result AddChuanDa(Cloth model)
  {
  ...
  public Result EditChuanDa(Cloth model,int id)
  {
         var result = new Result();
        //此处需优化，参数化处理
  ...
  public Result DeleteChuanDaById(int id)
  {
         var result = new Result();
        //此处需优化，参数化处理
